...
export function ParticipantHero({ participant }: ParticipantHeroProps) {
  ...
  return (
    <div className="relative overflow-hidden rounded-[2rem] bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800/60 shadow-sm p-6 md:p-10 transition-all">
      {/* Privacy Mode Banner */}
      <AnimatePresence>
         {isPrivacyProtected && (
            <motion.div 
               ...
               exit={{ opacity: 0, y: -20 }}
               className="absolute top-0 left-0 right-0 bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800/60 p-2 flex items-center justify-center gap-2 z-20"
            >
               <ShieldAlert className="h-4 w-4 text-slate-500 dark:text-slate-400" />
               <span className="text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400">Privacy Shield Active • Explicit Consent Required to View PII</span>
            </motion.div>
         )}
      </AnimatePresence>
      ...
      <div className="flex flex-col md:flex-row items-center md:items-start gap-8 relative z-10">
        ...
        <div className={`flex-1 space-y-5 text-center md:text-left relative z-10 ${isPrivacyProtected ? "mt-6 md:mt-10" : "mt-2 md:mt-0"}`}>
          ...
          <div className="grid grid-cols-2 lg:flex lg:items-center gap-y-4 lg:gap-x-8 text-sm font-medium text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-900 rounded-2xl p-4 border border-slate-100 dark:border-slate-800/50/50 w-fit mx-auto md:mx-0">
          ...
          <div className="flex flex-wrap items-center justify-center md:justify-start gap-2 pt-1">
            {isPrivacyProtected ? (
               <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-[0.5rem] bg-slate-100 text-slate-500 dark:text-slate-400 text-[10px] uppercase font-black tracking-widest border border-slate-200 dark:border-slate-800">
